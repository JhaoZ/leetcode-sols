...
class Solution:
    def countSmaller(self, nums: List[int]) -> List[int]:
        ...
        # Segment tree for counting frequencies
        size = len(ranks)
        freq_array = [0] * size
        segtree = SegmentTree(freq_array, merge=lambda a, b: a + b)

        res = []
        for num in reversed(nums):
            rank = ranks[num]
            count = segtree.query(0, rank - 1)  # How many smaller elements so far
            res.append(count)
            segtree.update(rank, segtree.query(rank, rank) + 1)  # Increase count
